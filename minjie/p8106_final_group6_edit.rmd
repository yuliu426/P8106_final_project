...
```{r include=TRUE, message=FALSE}
explainer_svm <- explain(model.svmr,
label = "svmr",
data = x_tr_1000,
y = as.numeric(y_tr_1000 == "change"),
verbose = FALSE)
...
p1 <- pdp::partial(model.svmr, pred.var = c("city_development_index"), grid.resolution = 10) %>% autoplot()
p1
```
...
grid.arrange(ice1.svmr, ice2.svmr, nrow = 1)
```
